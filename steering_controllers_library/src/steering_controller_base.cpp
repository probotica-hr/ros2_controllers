...
using ControllerTwistReferenceMsg =
  steering_controllers_library::SteeringControllerBase::ControllerTwistReferenceMsg;
...
{
SteeringControllerBase::SteeringControllerBase(steering_odometry::SteeringOdometryBase &odometry)
: controller_interface::ChainableControllerInterface(),
  odometryBase_(odometry)
{
}

controller_interface::CallbackReturn SteeringControllerBase::on_init()
{
  try
  {
...
controller_interface::CallbackReturn SteeringControllerBase::set_interface_numbers(
...
controller_interface::CallbackReturn SteeringControllerBase::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  params_ = param_listener_->get_params();
  odometryBase_.set_velocity_rolling_window_size(params_.velocity_rolling_window_size);
  ...
  if (params_.use_stamped_vel)
  {
    ref_subscriber_twist_ = get_node()->create_subscription<ControllerTwistReferenceMsg>(
      ...
      std::bind(&SteeringControllerBase::reference_callback, this, std::placeholders::_1));
  }
  else
  {
    ref_subscriber_unstamped_ = get_node()->create_subscription<geometry_msgs::msg::Twist>(
      ...
      std::bind(
        &SteeringControllerBase::reference_callback_unstamped, this, std::placeholders::_1));
  }
...
void SteeringControllerBase::reference_callback(
  const std::shared_ptr<ControllerTwistReferenceMsg> msg)
{
...
void SteeringControllerBase::reference_callback_unstamped(
...
SteeringControllerBase::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
SteeringControllerBase::state_interface_configuration() const
{
...
SteeringControllerBase::on_export_reference_interfaces()
{
...
bool SteeringControllerBase::on_set_chained_mode(bool chained_mode)
{
  // Always accept switch to/from chained mode
  return true || chained_mode;
}

controller_interface::CallbackReturn SteeringControllerBase::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::CallbackReturn SteeringControllerBase::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::return_type SteeringControllerBase::update_reference_from_subscribers()
{
...
controller_interface::return_type SteeringControllerBase::update_and_write_commands(
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (!std::isnan(reference_interfaces_[0]) && !std::isnan(reference_interfaces_[1]))
  {
    ...
    auto [traction_commands, steering_commands] =
      get_commands(last_linear_velocity_, last_angular_velocity_, params_.open_loop);
    if (params_.front_steering)
    {
      for (size_t i = 0; i < params_.rear_wheels_names.size(); i++)
      {
  ...
  orientation.setRPY(0.0, 0.0, odometryBase_.get_heading());

  // Populate odom message and publish
  if (rt_odom_state_publisher_->trylock())
  {
    rt_odom_state_publisher_->msg_.header.stamp = time;
    rt_odom_state_publisher_->msg_.pose.pose.position.x = odometryBase_.get_x();
    rt_odom_state_publisher_->msg_.pose.pose.position.y = odometryBase_.get_y();
    rt_odom_state_publisher_->msg_.pose.pose.orientation = tf2::toMsg(orientation);
    rt_odom_state_publisher_->msg_.twist.twist.linear.x = odometryBase_.get_linear();
    rt_odom_state_publisher_->msg_.twist.twist.angular.z = odometryBase_.get_angular();
